// quiz.js - minimal, works with backend /api/quiz endpoints produced earlier
const API = '/api';
let questions = [];
let current = 0;
let selected = null;
...
async function fetchQuestions(amount=10, source='opentdb', category='') {
  // Try OpenTDB first when source is 'opentdb'
  if (source === 'opentdb') {
    try {
      ...
      // Map OpenTDB format to our internal question format
      const mapped = j.results.map((item, idx) => {
        ...
        if (item.type === 'boolean') {
          ...
          // multiple choice — shuffle correct + incorrect
      ...
      console.warn('OpenTDB fetch failed, falling back to backend:', err);
      // fall through to backend
    }
  }

  // Fallback: ask backend for questions (existing behaviour)
  const resp = await apiFetch(`${API}/quiz/start?source=${source}&amount=${amount}`);
  if (!resp) return []; // redirected due to unauthenticated
...
async function start(amount = questionCount, category = '') {
  ...
  current = 0; selected = null; answers = [];
  renderQuestion();
}

// Hide modal and show quiz content
...
// Wire up preset question count buttons
...
// Start button for the custom input
...
document.getElementById('submitAnswer').addEventListener('click', async () => {
  ...
  answers.push({ questionId: questions[current].id, selected, correct: questions[current].correctAnswer });
  // next
  current++;
  selected = null;
  if (current >= questions.length) {
    ...
    try {
      await apiFetch(API + '/quiz/submit', {
        ...
        body: JSON.stringify({ score, totalQuestions: questions.length, questionsUsed: answers })
      });
    } catch(e){}
    // store locally and go to results
    sessionStorage.setItem('latestScore', JSON.stringify({score, total: questions.length, answers}));
...
// Restart button functionality - show modal again
...
// Load categories for categorySelect from OpenTDB
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', loadCategories); else loadCategories();
